...
description: Generate end-to-end traceability matrices from UML Workflow pipeline artifacts linking Scenario → Activity → Use Case → Entity → Security → Code → Test. Aggregates outputs from all pipeline steps to produce machine-readable JSON and human-readable Markdown reports with gap analysis. Inherits language settings from domain-model.json or usecase-output.json (Japanese/English/Bilingual). Use after Step 9 (test generation) for full forward+backward traceability, or after Step 6 (model validation) for pre-code coverage verification.
---
