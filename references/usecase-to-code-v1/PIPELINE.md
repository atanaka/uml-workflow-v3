...
> ```bash
> # ❌ 危険（環境依存）
> mkdir -p {WORK_DIR}/project/src/{components,pages,api}
>
> # ✅ 安全（必ずこちらを使う）
> mkdir -p {WORK_DIR}/project/src/components
> mkdir -p {WORK_DIR}/project/src/pages
> mkdir -p {WORK_DIR}/project/src/api
> ```
...
>   {WORK_DIR}/{project}/frontend/src/pages/AdminPage.tsx \
...
> client = open('{WORK_DIR}/{project}/frontend/src/api/client.ts').read()
> types  = open('{WORK_DIR}/{project}/frontend/src/types/index.ts').read()
> for name in targets:
...
> find {WORK_DIR}/{project}/frontend/src -type f | sort
>
...
> with open('{project}_domain-model.json') as f: dm=json.load(f)
> targets = [e['name'] for e in dm['entities'] if e['type'] in ('SYSTEM_ACTOR','MASTER_DATA')]
> base = '{WORK_DIR}/{project}/backend/src'
> for name in targets:
...
### Step 9.5: ⚠️ Backend Code Quality Requirements (MANDATORY)

The following requirements address real-world bugs observed in generated code (`library-management` project, 2026-05-22). Claude MUST apply these when generating backend code with Prisma + TypeScript + Express stack. These are not optional polishing — each one corresponds to a compile error, test failure, or developer-experience blocker that occurs on first run if omitted.

#### 9.5a. `backend/package.json` MUST include `prisma.seed`

**Why**: Without this key, `npx prisma db seed` and the auto-seed step of `npx prisma migrate reset` both fail with: `Error: To configure seeding in your project you need to add a "prisma.seed" property in your package.json`. Demo rehearsals that frequently reset the DB are blocked.

**Required addition** (sibling of `devDependencies`):
...
Use `tsx prisma/seed.ts` (not `ts-node`) since the rest of the generated stack uses tsx.

#### 9.5b. `AuditService.ts` MUST cast JSON fields to `Prisma.InputJsonValue`

**Why**: Prisma's generated types declare JSON columns (e.g., `auditLog.details`) as `NullableJsonNullValueInput | InputJsonValue | undefined`. Passing a plain `Record<string, unknown>` triggers TS2322 at compile time:

```
Type 'Record<string, unknown> | undefined' is not assignable to
...
export class AuditService {
  ...
  async record(event: { details?: Record<string, unknown> /* ... */ }) {
...
Apply the same cast to any other Prisma JSON column write (e.g., `metadata`, `payload`).

#### 9.5c. `rateLimit.ts` MUST be a no-op in `NODE_ENV=test`

**Why**: Integration tests call login endpoints many times in quick succession. With the production rate limiter active, the 6th+ call returns 429 and tests fail with `Expected: 200 / Received: 429`. The same friction hurts demo rehearsals on `development`.
...
export const loginLimiter = isTest
  ...
  : rateLimit({
      windowMs: 15 * 60_000,
      max: isDev ? 100 : 5,   // generous in dev so demos don't trip the limit
      standardHeaders: true,
      legacyHeaders: false,
    });

// Apply the same `isTest ? noop : rateLimit({...})` pattern to all other
// rate limiters (apiLimiter, registrationLimiter, etc.).
```

#### Verification

After generating backend code, Claude should `grep` to confirm each requirement applied:

```bash
# 9.5a
grep -A 2 '"prisma"' {WORK_DIR}/{project}/backend/package.json

# 9.5b (should print at least one match per generated AuditService.ts)
grep -n "Prisma.InputJsonValue" {WORK_DIR}/{project}/backend/src/services/AuditService.ts 2>/dev/null

# 9.5c
grep -n "NODE_ENV === 'test'" {WORK_DIR}/{project}/backend/src/middleware/rateLimit.ts 2>/dev/null
```

If any check fails, fix before proceeding to Step 10.
