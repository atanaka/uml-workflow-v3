...
### 4. Class UML Models — DUAL FORMAT, ALWAYS GENERATED BY DEFAULT

> ⭐ **クラスモデルは指示がなくても常にこの2形式を生成する（デフォルトON）。**
> 拡張子は両方とも **`.uml`**（`.xmi` は使わない）。両ファイルはセットで扱うこと。

| 形式 | Filename | 名前空間 / 用途 |
|---|---|---|
| **OMG 形式** | `{project}_class-model.omg.uml` | OMG 標準 XMI（EA / MagicDraw 等の汎用ツール向け） |
| **Eclipse UML2 形式** | `{project}_class-model.emf.uml` | EMF/Papyrus ネイティブ（Eclipse UML2 5.0.0） |

**Contents（両形式共通）:** 全クラス・属性・型・関連と多重度・列挙・ビジネスメソッド・制約。

#### 4a. OMG 形式 — `{project}_class-model.omg.uml`
UML 2.5.1 / OMG XMI。
```xml
...
#### 4b. Eclipse UML2 形式 — `{project}_class-model.emf.uml`
EMF/Papyrus ネイティブ。`xmi:id` を全要素に付与し、Eclipse UML2 名前空間を使う。
```xml
<?xml version="1.0" encoding="UTF-8"?>
<xmi:XMI xmi:version="2.1"
  ...
  <uml:Model xmi:id="_model_{project}" name="{project}">
    <packagedElement xmi:type="uml:Class" xmi:id="_c_order" name="Order">
      <ownedAttribute xmi:id="_c_order_id" name="orderId" type="_prim_string"/>
      <ownedAttribute xmi:id="_c_order_status" name="status" type="_e_orderstatus"/>
      <ownedOperation xmi:id="_c_order_confirm" name="confirm"/>
    </packagedElement>
    <packagedElement xmi:type="uml:Enumeration" xmi:id="_e_orderstatus" name="OrderStatus">
      <ownedLiteral xmi:id="_e_os_1" name="RECEIVED"/>
      <ownedLiteral xmi:id="_e_os_2" name="CONFIRMED"/>
    </packagedElement>
    <packagedElement xmi:type="uml:PrimitiveType" xmi:id="_prim_string" name="String"/>
  </uml:Model>
</xmi:XMI>
```

**Purpose:**
- OMG 形式: 汎用 UML ツール（Enterprise Architect, MagicDraw）との相互運用。
- Eclipse UML2 形式: Papyrus でのネイティブ編集・ラウンドトリップ。
...
4. {project}_class-model.omg.uml (OMG 形式 UML)
5. {project}_class-model.emf.uml (Eclipse UML2 形式 UML)
