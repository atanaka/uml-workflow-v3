...
`{project}_usecase-model.uml`

---

### Step 7.5: ⚠️ MANDATORY — Verify Output Completeness

**CRITICAL: After Steps 6 and 7 complete, Claude MUST verify that all use cases declared in `{project}_usecase-output.json` have a corresponding `.md` file in `usecase-specifications/`.**

このステップは LLM 出力の打ち切りによる「JSON には全 UC が記録されたが、.md ファイルが部分生成のまま完了扱いになる」事象（library-management で実発生）を検出するための必須ガードです。
...
from verify_step_outputs import verify_step2_outputs, assert_step2_complete

result = verify_step2_outputs('{project}')
...
if not result['is_complete']:
    ...
    if result['section_issues']:
        print('Section issues:', result['section_issues'])
    sys.exit(1)
print('✅ Step 2 output verification PASSED')
"
```

**検証失敗時の対応** (Claude が実行):

1. **不足している UC ID を特定** — `result["missing_uc_ids"]` に列挙される
2. **不足分の .md ファイルを生成** — 元の JSON から該当 UC を読み出し、Step 6b のテンプレートで個別に補完生成
3. **再度検証** — 上記コマンドを再実行し、`is_complete=True` になるまで繰り返す
4. **Section issues がある場合** — Cockburn 必須セクション（ユースケース情報・事前条件・事後条件・主成功シナリオ・拡張）の欠落を修正

**Claude はこの検証を絶対にスキップしてはならない。** 検証失敗のまま下流の Step に進むと SSoT 不整合が永久に残ります。
...
**Filename:** `{project}_usecase-model.uml`

> ⚠️ **このモデルの `.uml` はデフォルトでは生成しない。** uml-workflow-v3 で「クラスモデル以外も .uml 出力」を選んだ場合、または単独で明示的に指示された場合のみ生成すること（クラスモデルの2形式 .uml とは別扱い）。
...
5. {project}_usecase-model.uml (XMI)
